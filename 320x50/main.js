...
function animate() {
  ...
  tl1.to(['.copy1'], 0.5,{ autoAlpha: 1 }, 2);
  tl1.to(['.copy1'], 0.5,{ autoAlpha: 0 }, '+=3');
  tl1.to(['#copy3', '#cta'], 0.5,{ autoAlpha: 1 }, '+=0.5');
  tl1.to(['#copy2'], 0.5,{ autoAlpha: 1 }, '+=1');

  tl1.to(['#bkg'], 10,{ y: '-=50' }, 0);
  tl2.to(['#bike'], 14.5,{ y: '-=250', ease: "slow(0.7, 0.7, false)" }, 0);
  // tl2.call(bikeFloat, [5], '+=0')
}

function bikeFloat(loop) {
  for (let i = 0; i < loop; i++) {
    tl2.to(['#bike'], 1,{ y: '+=5', ease: "slow(0.7, 0.7, false)" }, '+=0');
    tl2.to(['#bike'], 1,{ y: '-=5', ease: "slow(0.7, 0.7, false)" }, '+=0');
  }
}
